...
def Inter_Bicubic(x, scale):
    ...
    for i in range(x_numpy.shape[0]):
        x_resize[i, 0, :, :] = cv2.resize(x_numpy[i, 0, :, :], (x_numpy.shape[3] * scale, x_numpy.shape[2] * scale),
                                          interpolation=cv2.INTER_CUBIC)
        x_resize[i, 1, :, :] = cv2.resize(x_numpy[i, 1, :, :], (x_numpy.shape[3] * scale, x_numpy.shape[2] * scale),
                                          interpolation=cv2.INTER_CUBIC)
        x_resize[i, 2, :, :] = cv2.resize(x_numpy[i, 2, :, :], (x_numpy.shape[3] * scale, x_numpy.shape[2] * scale),
                                          interpolation=cv2.INTER_CUBIC)
...
class IDN(nn.Module):
    def __init__(self, args):
        ...
        # Upsampler
        self.upsample = nn.ConvTranspose2d(nFeat, nChannel, stride=2, kernel_size=17, padding=8)
